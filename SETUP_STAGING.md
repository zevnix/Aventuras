...
1. `20240101000001_initial_schema.sql` (Esquema base de perfiles y usuarios)
2. `20240101000002_security_and_adult_child_links.sql` (Enums, solicitudes de vínculo temporal y constraints concurrentes)
3. `20240101000003_vertical_slice.sql` (Esquemas Data-driven, Misiones, Ledger Inmutable)
4. `20240101000004_functions_and_policies.sql` (Función `is_admin`, RPC `approve_mission`, y setup RLS)
5. `20240101000005_audit_fixes_and_security_enhancements.sql` (Endurecimiento de RLS, RPC de evidencias y control del rol Admin)
6. `20240101000006_player_balances_and_digital_missions.sql` (Vista materializada de XP/Coins/Gems y completado digital)
7. `20240101000007_identity_rls_fixes.sql` (RPC de búsqueda de usuario por username)
8. `20240101000008_auth_triggers.sql` (Creación automática de perfiles públicos vía Supabase Auth)
9. `20240101000009_dummy_kiro_and_progression_configs.sql` (Metadatos visuales "Kiro", configuraciones PENDING de thresholds).

*Nota:* Asegúrate de correr la migración `20240101000008_auth_triggers.sql` con permisos de *superuser* (`postgres`), ya que añade un trigger sobre la tabla protegida `auth.users`.
...
NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY=eyJhbG... (Tu clave publishable/pública)
```
*(Recuerda NUNCA usar la clave secreta / `service_role` en el `.env.local` de Next.js).*
